...
- `thread/readState/markAll` — experimental; enumerate every persisted thread selected by its provider, source, archive, cwd, and search scope, then atomically advance that ID set's read markers in one writer transaction. `readAt` is null when the scope matches no persisted threads.
